...
const Navbar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-white bg-black ">
        ...
        {
            nav && (
                <ul className="flex flex-col justify-center  absolute top-0 left-0 w-full  bg-gradient-to-b from-black to-gray-600 text-gray-500 pt-10"> 
                {
                    links.map(({id , link})=>  <li key={id} className="px-4 py-2 pb-4 text-xl cursor-pointer capitalize  font-medium ">
                        <Link to={link} smooth={true} duration={500}>{link}</Link>
                    </li>)
                   }
                </ul>
